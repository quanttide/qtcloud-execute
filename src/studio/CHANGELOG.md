...
自 beta.1 以来的完整能力（0.1.0-beta.13 为最后一个预发布版）：

- 真看板：状态泳道（四态）、任务卡拖拽自由来回、WIP 在制约束、列头「+」新建
- 任务全生命周期：新建（标题/优先级/描述）、详情编辑（标题/描述/状态/优先级）、删除（二次确认）、软归档演进中
- 右侧滑出面板交互；描述框自适应撑满
- 事件驱动状态层（TaskListBloc / BoardBloc），写操作结果信号化（成功刷新清单快照 / 失败 SnackBar 提示）
- 服务端 API（系统级网关 api.quanttide.com），CanvasKit 本地加载（零 Google 依赖，消除国内白屏）
...
- 删除/新建任务后左侧清单任务数徽章不更新：写操作成功后同步刷新清单层快照（与看板同源）

## [0.1.0-beta.10] - 2026-09-03

### Added

- 详情面板标题可编辑：标题输入框（与描述一并保存；清空保存则保留原标题）
