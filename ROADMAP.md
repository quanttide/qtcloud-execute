...
`Task`（id/title/description/status/priority）四字段模型。当前按
...
- [x] `models/task.dart`：Task（id/title/description/status/priority）+ 枚举（四态/四档）
...
- [x] 单测：Task JSON 往返 / TaskList 归属 / 仓储读写（fixture `test/fixtures/tasks.json`）

### 状态层（Bloc）

- [x] 依赖：flutter_bloc
- [x] `states/task_list_cubit.dart`：loadLists（动态）/ switchList
- [x] `states/board_cubit.dart`：loadTasks / updateTask / createTask（状态列投影）
- [x] 单测：Cubit 注入内存仓储（清单切换/自由移动/新建）

### 组件层（views）

- [x] `widgets/task_card.dart`：title + 优先级色点 + 优先级标签（状态由列表达）
- [x] `widgets/task_detail_dialog.dart`：详情弹窗（状态/优先级/描述——状态自由来回）
...
- [x] widget 测试：各组件（点击/排序/拖拽/WIP/新建）

### 页面收敛 + 旧代码删除

- [x] `screens/task_list_screen.dart`：分栏布局（左侧清单导航 + 右侧看板）
...
- [x] `Task.category` 字段与类别过滤器（`widgets/category_filter_bar.dart`）：任务不再分类，清单即隔离单元（provider/cli/studio 三层同步移除）
